...
export default {
  ...
  methods: {
    ...
    // return ;
      axios.get('/getAssetCapacityById', {
          params: {
    ...
    submitSupplierType() {
      ...
      axios.post('/updateAssetCapacity', this.formValues).then(response => {
        if (response.data.hasOwnProperty('success')) {
          if (response.data.success === true) {
            console.log(response.data.data)

            this.$router.replace('/assetCapacityList')
              .then(() => {
                this.$toast({
                  component: ToastificationContent,
                  position: 'top-right',
