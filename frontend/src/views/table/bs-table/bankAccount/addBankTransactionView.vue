    ...
    <b-row>
      ...
      <b-col md="6" v-if="employee_payment || hr_payments">
        <b-form-group label="To/From" label-for="To/From_emp1">
          <v-select
            id="To/From_emp1"
            v-model="formValues.to_from_array"
            :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
      ...
      >
        <b-form-group label="Sub Type" label-for="autosuggest__input">
          <vue-autosuggest
            v-model="formValues.sub_type_name"
            :suggestions="filteredOptions"
            :limit="10"
            :input-props="{
              id: 'autosuggest__input',
...
export default {
  ...
  methods: {
    ...
    submitDocument() {
      ...
      axios
        .post("/addBankAccountTransaction", formData)
        .then((response) => {
          if (response.data.hasOwnProperty("success")) {
            if (response.data.success === true) {
              ...
              this.$router
                .replace("/bank_accounts_transactions/" + this.$route.params.id)
                .then(() => {
                  this.$toast({
