  ...
  <div>
                    ...
                    <b-row>
                      ...
                      <b-col cols="6">
                        <b-button
                          ...
                          size="sm"
                          variant="primary"
                          :href="'/api/my_walletCsv?user_id='+user_id+'&amp;searchTerm='+ searchTerm+' &amp;limit='+exportLimit"
                          target="_blank"
                        >
                          <feather-icon icon="DownloadIcon" />
                        </b-button>
    ...
    <div class="container">
      <b-row class="">
        <b-col md="3" class="border border-primary text-white p-1 m-1 rounded" style="background: linear-gradient(118deg, #06608f, rgba(6, 96, 143, 0.7));">
          <strong>Total Cash In Circulation:  {{this.total_balance}} </strong>
        </b-col>

        <b-col md="3" class="border border-primary text-white p-1 m-1 rounded" style="background: linear-gradient(118deg, #06608f, rgba(6, 96, 143, 0.7));">
          <strong>Total Wallet Balance: {{this.total_wallet_balance}} </strong>
        </b-col>
      </b-row>
    </div>
          ...
          />
              ...
              <b-col md="12">
                <b-button
                  ...
                  variant="primary"
                  class="mr-1"
                  @click="page=1,searchTerm='',closeSearch(), getWallets()"
                >
                  <span  >GO</span>
                </b-button>
    ...
    >
      <template #cell(Options)="row">
        ...
        <b-form-group>
          <div class="d-flex align-items-center w-fit-content">
            <b-input-group>
              <b-input-group-prepend v-if="$ability.can('edit', 'expense_accounts')">
                <b-button
                  size="sm"
                  v-ripple.400="'rgba(113, 102, 240, 0.15)'"
                  variant="outline-primary text-success"
                  :to="{name: 'edit_my_expense_account', params: { id: row.item.id  } }"
              ...
              <b-input-group-append
                v-if="$ability.can('delete', 'expense_accounts')"
              >
                <b-button
                  size="sm"
              ...
              <b-input-group-append>
              <b-button v-if="$ability.can('view', 'expense_accounts')" 
                  size="sm"
              v-ripple.400="'rgba(40, 199, 111, 0.15)'"
      ...
      <template #cell(period)="data">
        ...
        {{ data.value | moment("DD,MMMM, YYYY - h:m A") }}
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      }, { key: 'account_name',label: 'Account Name', sortable: true, thClass: 'customHead' }, {
        key: 'period', label: 'Period', sortable: true, thClass: 'customHead',
      },
       { key: 'total',label: 'Total Expense', sortable: true, thClass: 'customHead' },
       { key: 'credit',label: 'credit', sortable: true, thClass: 'customHead' },

       { key: 'adjustment',label: 'adjustment', sortable: true, thClass: 'customHead' },
       { key: 'fuel_credit',label: 'fuel credit', sortable: true, thClass: 'customHead' },
        

       { key: 'balance',label: 'balance', sortable: true, thClass: 'customHead' },
  ...
  methods: {
    ...
    onClose() {
      ...
      this.$swal({
        ...
        if (result.value) {
            axios.post('/deleteExpenseAccount', {
            id: id,
          }
          ).then(response => {
    ...
    getWallets() {
      ...
      axios.get('/getExpenseAccounts', {
        ...
        .then(response => {
          ...
          this.total_wallet_balance = response.data.total_wallet_balance
          this.isBusy = false
