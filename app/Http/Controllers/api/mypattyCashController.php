...
class mypattyCashController extends Controller
{
    public function addTransaction(Request $request)
    {
        ...
        foreach (json_decode($request->items) as $item) {
            ...
            if (isset($item)) {
                ...
                $owner_wallet_name = Wallet::where('id', $request->id)->first();
                ...
                if (isset($item->assign_to_entity_id) && $item->assign_to_entity_id != '' && isset($item->assign_to_entity) && $item->assign_to_entity != '') {
                 if ($item->assign_to_category == 'Wallet'){
                    $owner_wallet_id = $item->wallet_id;
                    $walletName = Wallet::where('id', $item->wallet_id)->first();
                    $item->assign_to_entity = $walletName->account_name;
                    $item->description = "Transfer from {$owner_wallet_name->account_name} Wallet";
                    $item->wallet_id = $item->assign_to_entity_id;
                    $item->assign_to_entity_id = $owner_wallet_id;
                    $item->created_by = Auth::user()->name;
                    $item->credit_debit = 'Credit';
                    pattyCash::create(json_decode(json_encode($item), true));
                }
                }
            }
        }
    ...
    public function updateTransaction(Request $request){
        ...
        $messages = [
            ...
            'accept.required'  => 'Accept must be required.',
